...
class UniformPDFVolumetricSampler(Sampler):
    """A sampler that combines uniform, PDF, and volumetric sampling strategies."""

    def __init__(
        self,
        num_samples_uniform: int = 64,
        num_samples_pdf: int = 64,
        num_samples_volumetric: int = 32,
        num_upsample_steps: int = 4,
        base_variance: float = 64,
        single_jitter: bool = True,
    ) -> None:
        super().__init__()
        self.num_samples_uniform = num_samples_uniform
        self.num_samples_pdf = num_samples_pdf
        self.num_samples_volumetric = num_samples_volumetric
        self.num_upsample_steps = num_upsample_steps
        self.base_variance = base_variance
        self.single_jitter = single_jitter
    ...
    def generate_ray_samples(
        ...
        if ray_samples is None:
            ray_samples = self.uniform_sampler(ray_bundle, num_samples=self.num_samples_uniform)
        assert ray_samples is not None
        ...
        while total_iters < self.num_upsample_steps:
            ...
            new_samples = self.pdf_sampler(
                ...
                ray_samples,
                weights,
                num_samples=self.num_samples_pdf // self.num_upsample_steps,
            )
        ...
        # Apply volumetric sampling
        if ray_samples is not None:
            ray_samples = self.volumetric_sampler(ray_bundle, ray_samples, num_samples=self.num_samples_volumetric)
    ...
    @staticmethod
    def merge_ray_samples(ray_bundle: RayBundle, ray_samples_1: RaySamples, ray_samples_2: RaySamples):
        """Merge two sets of ray samples and return sorted index which can be used to merge sdf values
        Args:
            ray_samples_1 : ray_samples to merge
            ray_samples_2 : ray_samples to merge
        """
        ...
        ray_samples = ray_bundle.get_ray_samples(
            bin_starts=euclidean_bins[..., :-1, None],
            bin_ends=euclidean_bins[..., 1:, None],
            num_samples_per_ray=ray_samples_1.num_samples_per_ray + ray_samples_2.num_samples_per_ray,
            min_depth=torch.min(ray_samples_1.min_depth, ray_samples_2.min_depth),
            max_depth=torch.max(ray_samples_1.max_depth, ray_samples_2.max_depth),
        )
