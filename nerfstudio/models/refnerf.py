...
"""
Ref-NeRF implementation that combines Nerfacto and Ref-Nerf.
...
def l2_normalize(x, eps=torch.finfo(torch.float32).eps):
    ...
    return x / torch.sqrt(torch.fmax(torch.sum(x**2, dim=-1), torch.full_like(x, eps)))
...
def compute_weighted_mae(weights, normals, normals_gt):
    ...
    return (
        (weights * torch.arccos(torch.clamp(torch.sum(normals * normals_gt, -1), -one_eps, one_eps))).sum()
        / torch.sum(weights)
        * 180.0
        / np.pi
    )
...
@dataclass
class RefNerfactoModelConfig(ModelConfig):
    """Ref-Nerfacto Model Config"""
...
class RefNerfactoModel(Model):
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        if self.config.predict_normals:
            try:
                directions = ray_samples.frustums.directions  # old # ray_bundle.directions
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if self.training:
            ...
            if self.config.predict_normals:
                ...
                # tangent_dot_normals = torch.sum(tangent * normals, dim=-1)
