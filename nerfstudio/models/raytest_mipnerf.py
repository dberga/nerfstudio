"""
Implementation of combined-mip-NeRF.
"""
...
from nerfstudio.model_components.ray_samplers import UniformPDFVolumetricSampler, UniformSampler
...
class CominedMipNerfModel(Model):
    """mip-NeRF model
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sampler_combined = UniformPDFVolumetricSampler(
            num_samples_uniform=self.config.num_coarse_samples,
            num_samples_pdf=self.config.num_importance_samples,
            num_samples_volumetric=0,  # Adjust volumetric sampling if needed
        )
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        # combined sampling
        ray_samples_combined = self.sampler_combined(ray_bundle, ray_samples_uniform, weights_coarse)

        # Second pass:
        field_outputs_fine = self.field.forward(ray_samples_combined)
        if self.config.use_gradient_scaling:
            field_outputs_fine = scale_gradients_by_distance_squared(field_outputs_fine, ray_samples_combined)
        weights_fine = ray_samples_combined.get_weights(field_outputs_fine[FieldHeadNames.DENSITY])
        rgb_fine = self.renderer_rgb(
            rgb=field_outputs_fine[FieldHeadNames.RGB],
            weights=weights_fine,
        )
        accumulation_fine = self.renderer_accumulation(weights_fine)
        depth_fine = self.renderer_depth(weights_fine, ray_samples_combined)
