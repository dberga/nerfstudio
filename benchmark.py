...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(columns=['experiment_name', 'ckpt_path', 'fps', 'fps_std', 'lpips',
                      'lpips_std', 'psnr', 'psnr_std', 'ssim', 'ssim_std']).set_index('experiment_name')
    ...
    for results_path in list_results:
        ...
        series_results = results_row.append(
            pd.Series([ckpt_path], index=['ckpt_path']))
    ...
    if not df.empty:
        ...
        list_nonessential = [i for i in list(
            df.columns) if i not in list_essential]
        ...
        output_benchmark_csv = os.path.join(
            scene_path, f'benchmark_{scene_name}.csv')
        df.to_csv(output_benchmark_csv, sep=',')
        shutil.copy(output_benchmark_csv, os.path.join(
            BENCHMARKS_FOLDER, f'benchmark_{scene_name}.csv'))
        # write .md
        output_benchmark_md = os.path.join(
            BENCHMARKS_FOLDER, f'benchmark_{scene_name}.md')
